    ...
    if (cart_cell.length) {
      ...
      jQuery.each(cart, function(key, value) {
        if (key === 'items') {
          ...
          if (value.length) {
            jQuery(".mini_cart_items").css({"display": "block"});
            ...
            jQuery.each(value, function(i, item) {
              ...
              jQuery('<li class="cart_item bottom"><div class="cart_img"><a href="' + item.url + '"><img src="' + item.image + '" alt="{{ item.image.alt }}" style="width: 90px;"></a></div><div class="cart_info"><a href="' + item.url + '">' + item.title + '</a><span><span class="mini_cart_qty">' + item.quantity + 'x ' + Shopify.formatMoney(item.price, formatMoney) + '</span></span></div><div class="cart_remove"><a href="javascript:void(0);" onclick="Shopify.removeItem(' + item.variant_id + ')"><i class="ion-android-close"></i></a></div></li>').appendTo(table);
            });
          }
          else {
            jQuery(".mini_cart_items").css({"display": "none"});
